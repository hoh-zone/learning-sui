...
1. 进入 `src/10_programmability/code/programmability_lab/`。
...
1. 参考 `src/11_patterns/code/patterns_lab/` 中的 `fun init`（第十章与十一章交界，可对照阅读）。
2. 在 `programmability_lab` 或副本包中，为模块添加 `init`，只做**一件**事（如发一个 `AdminCap` 给部署者）。
3. **验收**：`sui move test` 或本地构建通过；能解释 `init` 何时运行一次。

## 实战三：Clock 只读调用（链下组合）

1. 阅读第十章「Epoch 与时间」与 `src/20_advanced_topics/code/advanced_lab/sources/clock_probe.move`。
