...
在 Sui Move 中，`key` 能力是定义<b>对象（Object）</b>的核心标志。一个结构体只有拥有 `key` 能力，才能作为独立的链上对象存在，拥有全局唯一的标识符，并参与 Sui 的所有权和存储模型。理解 `key` 能力是掌握 Sui 对象系统的第一步。

## key 的历史演变

在早期的 Move 语言（Diem/Aptos 版本）中，`key` 能力表示一个类型可以作为<b>全局存储的顶层资源（Resource）</b>存在。拥有 `key` 的结构体可以通过 `move_to`、`move_from` 等操作存储到账户地址下。
