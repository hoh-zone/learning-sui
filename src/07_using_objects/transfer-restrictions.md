...
同类「类型须由当前模块定义」的约束（验证器中的**内部约束**）也适用于其他 Sui 标准 API，例如 `sui::event::emit<T>`：泛型参数 `T` 必须由调用方所在模块定义，否则验证器会报错。其目的与转移限制一致：保证关键操作的类型由可信模块控制。专节说明见[内部约束](internal-constraint.md)。
