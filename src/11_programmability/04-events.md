...
本节对应 [§11.1](01-sui-framework.md) 中的 **`sui::event`**：`emit` 把结构化数据挂到**交易效果**上，供索引器与前端消费；**不**占用对象动态字段。事件类型须 **`copy + drop`**，且须在**本模块**定义（内部约束见链接）。

- **前置**：[§11.1](01-sui-framework.md)、[§11.2](02-transaction-context.md)（常用 `ctx.sender()` 填入事件）  
...
在 [§11.1](01-sui-framework.md) 的框架地图中，把 **`event`** 与 **`object` / `transfer`** 区分开：**事件不是对象字段**，也不替代链上状态。
