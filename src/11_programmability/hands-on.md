...
4. **验收**：编译通过；能说明为何事件类型需要 `copy + drop`，并对照 [§11.1](01-sui-framework.md) 中 `event` 在框架中的位置。
...
1. 阅读 [§11.5 · Epoch 与时间](05-epoch-and-time.md) 与 `src/21_advanced_topics/code/advanced_lab/sources/clock_probe.move`（若仓库中有）。
...
2. 分别说明：若用 **`VecMap`** 会有什么问题；若用 **`Table`** 优势在哪（对照 [§11.1](01-sui-framework.md) 集合表与 [§11.6](06-collections.md)、[§11.10](10-dynamic-collections.md)）。
3. **验收**：书面回答即可，无需长代码。

## 实战五：框架地图自测（全章）

1. 打开 [§11.1](01-sui-framework.md)，遮住「集合选型」表，在纸上默写 `VecMap` / `Table` / `ObjectTable` / `Bag` 四类在「键同质、值是否对象、数据住哪」上的区别。
2. **验收**：能口述无误后再回到表格核对。
