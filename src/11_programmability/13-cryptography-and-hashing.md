...
本节对应 [§11.1](01-sui-framework.md) 中的 **`sui::crypto/*`** 与相关哈希模块：在链上做哈希、签名验证、ZK 相关原语等。**安全模型**与 Gas 成本与具体算法绑定，生产环境务必对照**当前**框架源码与审计建议。

- **前置**：[§11.1](01-sui-framework.md)、[§11.12](12-bcs.md)（字节与承诺常一起出现）  
- **后续**：[第十八章 · 安全](../19_security/00-index.md)（误用密码学的常见坑）  
