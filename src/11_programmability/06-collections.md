...
本节对应 [§11.1](01-sui-framework.md) **集合选型表**中的 **`VecMap` / `VecSet`**：数据存放在**宿主对象内部**，实现简单、适合**小规模**；与 [§11.10](10-dynamic-collections.md) 的 `Table` / `Bag` 等（**动态字段**后端、可扩展）形成对照。

- **前置**：[§11.1](01-sui-framework.md)（选型表）、[第六章 · Vector](../06_move_intermediate/02-vector.md)（底层仍是 `vector` 语义）  
- **后续**：[§11.10](10-dynamic-collections.md)（数据变大时迁移思路）  
...
回到 [§11.1](01-sui-framework.md) 的集合对比表，把「**对象内**」与「**动态字段**」两行对照记忆，选型会更快。
