...
| 节 | 主题 | 你将学到 |
|---|------|---------|
| 17.1 | 升级机制与 UpgradeCap | 升级原理、三步流程、兼容性规则、CLI 操作 |
| 17.2 | 升级策略 | 四种内置策略、收紧操作、自定义策略（时间锁/多签） |
| 17.3 | 版本化共享对象 | 包级、对象级、混合三种版本化模式 |
| 17.4 | 数据迁移与向前兼容 | 动态字段扩展、Bag/Anchor 模式、用户对象迁移 |
| 17.5 | 实战：Hero 游戏完整升级 | 从 V1 到 V2 的完整案例与 CLI 验证 |
