...
**要点复述**：本路径对应 [§15.2](02-registry-otw.md) 的 **`new_currency_with_otw` + `finalize`**；**`TreasuryCap`** 是铸币唯一入口（在供应策略允许的前提下）。
...
3. **验收**：能向他人说明 **`MetadataCap` 与 `TreasuryCap` 的职责分离**（见 [§15.3](03-coin-metadata.md)）。
...
3. **验收**：写出 **「提交封禁」与「当前 epoch 是否已生效」** 的差异说明（见 [§15.8](08-regulated-denylist.md)）。
...
3. **验收**：能解释 **`ActionRequest.approvals`** 与 **`TokenPolicy.rules`** 的匹配关系（见 [§15.10](10-token-policy.md)）。

---

## 实战六（进阶）：嵌入式金库

1. 仿 [§15.13](13-balance-vault-patterns.md)，实现 **`deposit` / `withdraw`**，并补 **`sui move test`** 或用 **测试场景** 验证 **余额守恒**。  
